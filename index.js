const { Client, Collection, Events, ModalBuilder, ActionRowBuilder, EmbedBuilder, ButtonStyle, TextInputBuilder, TextInputStyle, ButtonBuilder, GatewayIntentBits, Partials, ActivityType } = require('discord.js');
const fs = require('fs');
...
client.once(Events.ClientReady, () => {
    ...
    client.user.setPresence({
        activities: [{ name: 'to Customers!', type: ActivityType.Listening }],
        status: 'Away'
    });
});
...
client.on(Events.InteractionCreate, async(interaction) => {
    ...
    if (interaction.isModalSubmit()) {
        if (interaction.customId == 'nat2k15developmetntrateiscoolfr') {
            ...
            interaction.reply({ content: "Your review has been submitted, Thank you!", ephemeral: true });
...
client.openhandesomemodal = async function(interaction) {
    ...
    const service = new TextInputBuilder()
        ...
        .setLabel("Rate our service 1 to 5")
        .setPlaceholder("1-5")
        .setMinLength(1)
        .setMaxLength(1)
